...
When **wath-core** is running with `GITHUB_TOKEN` set, a background merge poller records merges automatically (every 30s). With `CURSOR_API_KEY` set, a drift poller audits for standard version drift (every 1 min) and launches remediation onboard runs. Before launching agents, Wath reconciles the ledger against GitHub (open Wath PRs and orphan `cursor/*` branches) so drift remediation reuses in-flight work instead of opening duplicate branches. Stale `activeRun` files from redeploys are cleared on startup.

**Drift remediation** uses a dedicated validate-on-`main` agent prompt: read `version_notes` / `version_changelog` in `standards/registry.yaml`, run conformance on existing artifacts, and apply a **minimal diff** — not a full re-integrate. Wath skips the integrate phase and does **not** create a `cursor/*` branch when verify passes with no artifact changes; the ledger bumps to the new registry version **without a PR**. Orphan branches from prior runs are deleted when resolved this way. When files do change, the agent creates one integration branch and uses the drift PR template (`wath-drift-remediation.md`). Registry version (marketplace counter) can differ from `standard.yaml` content version; document each bump in `version_changelog`.

You can also poll on demand:
