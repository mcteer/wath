...
description: >
  ...
  "dynamic secrets" is never used. Owned by: Security Engineering (CODEOWNERS).
...
| `policy.hcl` (least-privilege) | Configuring the DB connection with **root** credentials |
| the auth role binding (e.g. `auth/kubernetes/role/<app>`) | Rotating the root credential |
| consumer wiring (VSO CR or Agent Injector annotations) | Network/firewall path from Vault to the DB |
| the database **role parameters** (TTLs, creation statements) | |
| the shipped CI verification workflow | |
...
runtime from the repo / environment and apply:
...
## 5. The rules (MUST — each maps to a conformance check of the same ID)
...
## 6. What this standard does *not* mechanize (human ratification)
...
## 7. How this standard is enforced (the pairing)
...
3. **Tier-2** — the shipped CI workflow (VDS-008) re-runs the same gate in the team's real
   environment.
4. **Human ratification** — the two merge gates cover the residue from §6.
